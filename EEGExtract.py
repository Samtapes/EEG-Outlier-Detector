...
def extractFeatures(eegData, fs):
    #######
    # COMPLEXITY FEATURES
    ...
    #Tsalis Entropy (n=10)
    tsalisRes = tsalisEntropy(eegData, bin_min=-200, bin_max=200, binWidth=2, orders=list(range(1,10+1)))

    # Subband Information Quantity
    # delta (0.5–4 Hz)
    eegData_delta = filt_data(eegData, 0.5, 4, fs)
    ShannonRes_delta = shannonEntropy(eegData_delta, bin_min=-200, bin_max=200, binWidth=2)
    # theta (4–8 Hz)
    eegData_theta = filt_data(eegData, 4, 8, fs)
    ShannonRes_theta = shannonEntropy(eegData_theta, bin_min=-200, bin_max=200, binWidth=2)
    # alpha (8–12 Hz)
    eegData_alpha = filt_data(eegData, 8, 12, fs)
    ShannonRes_alpha = shannonEntropy(eegData_alpha, bin_min=-200, bin_max=200, binWidth=2)
    # beta (12–30 Hz)
    eegData_beta = filt_data(eegData, 12, 30, fs)
    ShannonRes_beta = shannonEntropy(eegData_beta, bin_min=-200, bin_max=200, binWidth=2)
    # gamma (30–100 Hz)
    eegData_gamma = filt_data(eegData, 30, 100, fs)
    ShannonRes_gamma = shannonEntropy(eegData_gamma, bin_min=-200, bin_max=200, binWidth=2)

    # Cepstrum Coefficients (n=2)
    CepstrumRes = mfcc(eegData, fs,order=2)

    # Lyapunov Exponent
    LyapunovRes = lyapunov(eegData)

    # Fractal Embedding Dimension
    HiguchiFD_Res  = hFD(eegData[0,:,0],3)

    # Hjorth Mobility
    # Hjorth Complexity
    HjorthMob, HjorthComp = hjorthParameters(eegData)

    # False Nearest Neighbor
    FalseNnRes = falseNearestNeighbor(eegData)

    # ARMA Coefficients (n=2)
    armaRes = arma(eegData,order=2) 
    ...
    # Median Frequency
    medianFreqRes = medianFreq(eegData,fs)
    ...
    # γ band Power
    bandPwr_gamma = bandPower(eegData, 30, 100, fs)
    ...
    # Voltage < 5μ
    volt05_res = eegVoltage(eegData,voltage=5)
    # Voltage < 10μ
    volt10_res = eegVoltage(eegData,voltage=10)
    # Voltage < 20μ
    volt20_res = eegVoltage(eegData,voltage=20)

    # Diffuse Slowing
    df_res = diffuseSlowing(eegData)

    # Spikes
    minNumSamples = int(70*fs/1000)
    spikeNum_res = spikeNum(eegData,minNumSamples)

    # Delta burst after Spike
    deltaBurst_res = burstAfterSpike(eegData,eegData_delta,minNumSamples=7,stdAway = 3)

    # Sharp spike
    sharpSpike_res = shortSpikeNum(eegData,minNumSamples)

    # Number of Bursts
    numBursts_res = numBursts(eegData,fs)

    # Burst length μ and σ
    burstLenMean_res,burstLenStd_res = burstLengthStats(eegData,fs)

    # Burst Band Power for δ
    burstBandPwrAlpha = burstBandPowers(eegData, 0.5, 4, fs)

    # Number of Suppressions
    numSupps_res = numSuppressions(eegData,fs)

    # Suppression length μ and σ
    suppLenMean_res,suppLenStd_res = suppressionLengthStats(eegData,fs)

    #######
    # CONNECTIVITY FEATURES

    # Coherence - δ
    coherence_res = coherence(eegData,fs)
